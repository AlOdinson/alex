...
assert.match(css, /\.floating-drawing-controls \.color-control[\s\S]*?border-radius:\s*50%/, 'Color control must be circular');
assert.match(css, /title\^=\"Прозрачность\"[\s\S]*?repeating-conic-gradient/, 'Opacity control must use a transparency checker track');
assert.match(css, /title\^=\"Толщина\"[\s\S]*?linear-gradient\([^\n]*#cbd5e1/, 'Width dots must be connected with a light line');
const dotStops = css.match(/radial-gradient\(circle at [^,]+,\s*#8a96a8 0 4\.5px/g) ?? [];
assert.equal(dotStops.length, 12, 'Width track must render exactly 12 equal-size dots');
assert.match(css, /::-webkit-slider-thumb[\s\S]*?width:\s*13px;[\s\S]*?height:\s*13px;/, 'Selected width dot must stay compact and circular');
