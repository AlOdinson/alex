...
const languageFixUrl = new URL('../src/floating-drawing-controls-language-fix.css', import.meta.url);

assert.match(main, /import ['\"]\.\/floating-drawing-controls\.css['\"];/, 'Floating drawing controls CSS must be loaded');
assert.match(main, /import ['\"]\.\/floating-drawing-controls-language-fix\.css['\"];/, 'Language-independent floating control overrides must be loaded');
...
assert.ok(fs.existsSync(languageFixUrl), 'Language-independent slider override stylesheet must exist');
...
const languageFix = fs.readFileSync(languageFixUrl, 'utf8');
...
assert.match(languageFix, /\.eyedropper-button \+ \.compact-slider[\s\S]*?--opacity-stop[\s\S]*?repeating-conic-gradient/, 'Opacity control must use checkerboard transparency with dynamic fill');
assert.match(languageFix, /\.compact-slider \+ \.compact-slider[\s\S]*?linear-gradient\(90deg, #cbd5e1, #cbd5e1\)/, 'Width dots must be connected with a light line');
assert.doesNotMatch(languageFix, /title\^=/, 'New slider overrides must not depend on translated title text');
const dotStops = languageFix.match(/radial-gradient\(circle at [^,]+,\s*#8a96a8 0 4px/g) ?? [];
assert.equal(dotStops.length, 12, 'Width track must render exactly 12 equal-size dots');
assert.match(languageFix, /\.compact-slider \+ \.compact-slider[\s\S]*?strong\s*\{\s*display:\s*none\s*!important;/, 'Width numbers must not be displayed');
