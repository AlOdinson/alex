...
assert.match(historyCss, /@supports \(-webkit-touch-callout:\s*none\)/, 'iPhone/iPad must have an explicit WebKit glass fallback independent of pointer reporting');
assert.match(accessoryCss, /@supports \(-webkit-touch-callout:\s*none\)/, 'iPhone/iPad accessories and dock must have an explicit WebKit fallback');
assert.ok(historyCss.lastIndexOf('@supports (-webkit-touch-callout: none)') > historyCss.lastIndexOf('@media (max-width: 760px)'), 'iOS history fallback must be last so mobile geometry rules cannot mute its optics');
assert.ok(accessoryCss.lastIndexOf('@supports (-webkit-touch-callout: none)') > accessoryCss.lastIndexOf('@media (max-width: 760px)'), 'iOS accessory fallback must be last in the cascade');
assert.match(historyCss, /@supports \(-webkit-touch-callout:\s*none\)[\s\S]*?\.dock-history-accessories\s*\{[\s\S]*?rgba\(145, 203, 255, 0\.34\)/, 'iOS history glass must have a clearly visible cold reflection even if blur is weak');
assert.match(accessoryCss, /@supports \(-webkit-touch-callout:\s*none\)[\s\S]*?\.dock-style-eyedropper-button::before,[\s\S]*?\.dock-style-preset-button::before\s*\{[\s\S]*?opacity:\s*1/, 'iOS square tiles must render their premium specular layer at full strength without hover');
assert.match(accessoryCss, /@supports \(-webkit-touch-callout:\s*none\)[\s\S]*?\.board-tool-dock\s*\{[\s\S]*?rgba\(255, 255, 255, 0\.72\)/, 'iOS matte dock must remain visibly frosted even over a white board');
assert.match(accessoryCss, /@media \(max-width:\s*900px\)[\s\S]*?\.dock-style-eyedropper-button::before,[\s\S]*?\.dock-style-preset-button::before\s*\{[\s\S]*?opacity:\s*1/, 'Phone viewport fallback must keep premium tile reflections visible on non-iOS mobile browsers');
assert.match(accessoryCss, /@media \(max-width:\s*900px\)[\s\S]*?\.board-tool-dock\s*\{[\s\S]*?rgba\(255, 255, 255, 0\.72\)/, 'Phone viewport fallback must make the matte dock visibly distinct from the white board');
