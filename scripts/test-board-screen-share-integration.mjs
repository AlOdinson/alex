...
has(/screenShare\.updateBoardLayout/, 'Fabric transforms must broadcast layout through the screen-share protocol.');
has(/before:transform[\s\S]*?isBoardScreenShareObject\(target\)/, 'before:transform must short-circuit durable object leases for the live screen.');
