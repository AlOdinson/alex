...
if old_tools in toolbar:
    toolbar = toolbar.replace(old_tools, new_tools, 1)

if 'function DockToolIcon({ id })' not in toolbar:
    icons = r'''
...
'''
    toolbar = replace_once(toolbar, '\n\nfunction ExportIcon() {', icons + '\n\nfunction ExportIcon() {', 'dock icon insertion')

if 'className="tool-group main-tools"' in toolbar:
    start = toolbar.index('        <div className="tool-group main-tools" aria-label="Инструменты">')
    next_group = '        <div className="tool-group compact" aria-label="Отмена и возврат">'
    end = toolbar.index(next_group, start)
    toolbar = toolbar[:start] + toolbar[end:]

if 'className="board-tool-dock"' not in toolbar:
    toolbar = replace_once(
        toolbar,
        '  return (\n    <header className="toolbar-shell">',
        '  return (\n    <>\n      <header className="toolbar-shell">',
        'toolbar fragment opening',
    )
    ending = '      </div>\n    </header>\n  );\n}\n'
    dock = r'''      </div>
      </header>
...
'''
    toolbar = replace_once(toolbar, ending, dock, 'toolbar dock ending')

toolbar_path.write_text(toolbar)

palette_path = Path('src/components/ShapePalette.jsx')
palette = palette_path.read_text()
if 'const openAbove = availableAbove > availableBelow;' not in palette:
    old = r'''      const preferredLeft = rect.left + viewportOffsetLeft - 76;
      const left = Math.min(
    ...
    new = r'''      const preferredLeft = rect.left + viewportOffsetLeft - 76;
      const left = Math.min(
        Math.max(viewportOffsetLeft + EDGE_GAP, preferredLeft),
        viewportOffsetLeft + viewportWidth - width - EDGE_GAP,
      );
    ...
    palette = replace_once(palette, old, new, 'shape palette placement')
    marker = "        '--shape-palette-max-height': `${placement.maxHeight}px`,\n"
    palette = replace_once(
        palette,
        marker,
        marker + "        transform: placement.openAbove ? 'translateY(-100%)' : undefined,\n        transformOrigin: placement.openAbove ? 'bottom left' : 'top left',\n",
        'shape palette transform',
    )
    palette_path.write_text(palette)

i18n_path = Path('src/i18n.js')
i18n = i18n_path.read_text()
if "'Картинка': 'Image'" not in i18n:
    marker = "  'Добавить картинку': 'Add image',\n"
    i18n = replace_once(i18n, marker, marker + "  'Картинка': 'Image',\n", 'i18n image')
    i18n_path.write_text(i18n)

styles_path = Path('src/styles.css')
styles = styles_path.read_text()
style_marker = '/* Alex Board 1.35.0 — reference-style bottom tool dock */'
if style_marker not in styles:
    styles += r'''

/* Alex Board 1.35.0 — reference-style bottom tool dock */
.board-tool-dock {
  position: absolute;
  z-index: 42;
  left: 50%;
...
.board-tool-dock::-webkit-scrollbar { display: none; }

.board-tool-dock .dock-tool-button {
  flex: 0 0 62px;
  width: 62px;
  height: 58px;
  padding: 0;
...
.board-tool-dock .dock-tool-button > span,
.board-tool-dock .dock-tool-content {
  width: 100%;
  height: 100%;
  display: flex;
...
.board-tool-dock .dock-tool-button:hover { background: #f5f3ff; }
.board-tool-dock .dock-tool-button:active:not(:disabled) { transform: translateY(1px); }

.board-tool-dock .dock-tool-button.active {
  background: #ede9fe;
  color: #5b21b6;
...
.board-tool-dock .dock-tool-button.active .dock-tool-label { color: #6d28d9; }
.board-tool-dock .dock-shape-anchor { flex: 0 0 auto; }

.board-tool-dock .image-upload-button {
  position: relative;
  display: flex;
  cursor: pointer;
}

.board-tool-dock .image-upload-button.disabled {
  cursor: not-allowed;
  opacity: 0.45;
}

.board-tool-dock .image-file-input { display: none; }
...
@media (max-width: 760px) {
  ...
  .dock-tool-icon { width: 24px; height: 24px; }
  .dock-tool-label { max-width: 50px; font-size: 8.5px; }
}
'''
...
command = 'node scripts/test-bottom-tool-dock.mjs && '
if command not in package:
    package = replace_once(package, '"test:sync": "', '"test:sync": "' + command, 'package test:sync')
